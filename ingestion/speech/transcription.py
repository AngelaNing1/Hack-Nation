    ...
    def _to_transcript(
        ...
        for index, turn in enumerate(turns):
            ...
            for raw_word in turn.get("words", []):
                token = str(getattr(raw_word, "word", None) or raw_word["word"]).strip()
                if not token:
                    continue
                ...
                start = float(getattr(raw_word, "start", None) or raw_word["start"])
                end = float(getattr(raw_word, "end", None) or raw_word["end"])
                words.append(
                    TranscriptWord(
                        text=token,
                        start_seconds=start,
