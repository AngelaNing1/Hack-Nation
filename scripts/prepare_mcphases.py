...
def _stream_table(root: Path) -> Path:
    """Pick the stream table to ingest from an mcPHASES root."""
    if root.is_file():
        return root
    for name in STREAM_FILE_CANDIDATES:
        if (root / name).exists():
            return root / name
    candidates = sorted(root.glob("*.csv"))
    if not candidates:
        raise McPhasesDataNotFoundError(
            f"mcPHASES root '{root}' contains no stream table. Expected one of "
            f"{', '.join(STREAM_FILE_CANDIDATES)} or any *.csv file."
        )
    return candidates[0]
