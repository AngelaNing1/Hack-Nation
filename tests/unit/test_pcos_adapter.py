...
def test_symptom_only_androgenic_score_is_qualified():
    ...
    assert scores["androgenic"].evidence_qualifier == "androgenic-symptom evidence"


def test_biochemically_supported_androgenic_score_is_not_qualified():
    scores = compute_domain_scores(
        {"total_testosterone": 2.0, "shbg": -1.0, "free_testosterone": 1.0}
    )
    assert scores["androgenic"].evidence_qualifier is None
