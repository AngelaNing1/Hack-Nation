...
def test_specs_come_entirely_from_the_registry(registry):
    ...
    androgenic = specs["androgenic"]
    assert androgenic.symptom_only_qualifier == "androgenic-symptom evidence"
    assert androgenic.min_coverage_to_report == pytest.approx(0.25)
    shbg = next(f for f in androgenic.features if f.code == "shbg")
...
def test_coverage_is_the_observed_weight_fraction(fitted_scorer):
    ...
    spec = fitted_scorer.specs["androgenic"]
    observed = {"total_testosterone": 40.0, "hirsutism": 1.0}
    row = pd.DataFrame([observed])

    score = fitted_scorer.score_frame(row)["androgenic"][0]

    expected_weight = sum(f.weight for f in spec.features if f.code in observed)
    assert score.coverage == pytest.approx(expected_weight / spec.total_weight)
    assert sorted(score.observed_features) == sorted(observed)
...
def test_direction_flips_the_contribution(fitted_scorer):
    ...
    high_score = fitted_scorer.score_frame(high)["androgenic"][0]
    low_score = fitted_scorer.score_frame(low)["androgenic"][0]
...
def test_weights_are_honored(fitted_scorer):
    ...
    heavy_score = fitted_scorer.score_frame(heavy)["androgenic"][0]
    light_score = fitted_scorer.score_frame(light)["androgenic"][0]

    assert heavy_score.coverage > light_score.coverage


def test_symptom_only_observation_sets_the_evidence_qualifier(fitted_scorer):
    ...
    assert score.evidence_qualifier == "androgenic-symptom evidence"
    assert any("self-reported symptoms only" in w for w in score.warnings)


def test_biochemical_observation_does_not_set_the_qualifier(fitted_scorer):
    row = pd.DataFrame([{"hirsutism": 1.0, "total_testosterone": 55.0}])
    score = fitted_scorer.score_frame(row)["androgenic"][0]

    assert score.evidence_qualifier is None
    assert not any("symptoms only" in w for w in score.warnings)
...
def test_manifest_records_weights_and_thresholds(fitted_scorer):
    ...
    androgenic = manifest["domains"]["androgenic"]
    assert androgenic["symptom_only_qualifier"] == "androgenic-symptom evidence"
